...
#define NHSCREEN_HEIGTH [UIScreen mainScreen].bounds.size.height
#endif
...
@property (nonatomic, strong) UIButton *shiftBtn,*charSymSwitch,*deleteCharBtn,*safeBtn,*loginBtn;
...
+ (nullable)keyboardWithType:(NHKBType)type andSelectedBtnTitle:(NSString *) title {
    return [[NHKeyboard alloc] initWithFrame:CGRectMake(0, 0, NHSCREEN_WIDTH, NHKBH) withType:type andSelectedBtnTitle:title];
}

- (id)initWithFrame:(CGRect)frame withType:(NHKBType)type andSelectedBtnTitle:(NSString *) title {
    self = [super initWithFrame:frame];
    if (self) {
        self.type = type;
        self.selectedBtnTitle=title;
        [self _initSetup];
    }
    return self;
}
...
//    CGFloat iconH = NHICONH;
    CGFloat kbH = NHKBH;
    UIColor *bgColor = NHColor(233, 238,245);
    CGRect bounds = self.bounds;
    bounds.size.height = kbH+54;
    self.bounds = bounds;
    NSLog(@"%@%@",self.superview,NSStringFromCGRect(self.frame));
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    ...
    if (self.showSymbol) {
        ...
        [self charSymbolSwitch:self.charSymSwitch];
...
#define Characters @[@"1",@"2",@"3",@"4",@"5",@"6",@"7",@"8",@"9",@"0",@"q",@"w",@"e",@"r",@"t",@"y",@"u",@"i",@"o",@"p",@"a",@"s",@"d",@"f",@"g",@"h",@"j",@"k",@"l",@"z",@"x",@"c",@"v",@"b",@"n",@"m"]
...
- (void)setupASCIICapableLayout:(BOOL)init {
    ...
    //键盘视图的大小
    CGRect boundsView = self.bounds;
    
    if (!self.showSymbol) {
        charSets = Characters;
        rangs = @[@10,@20,@29,@36];
        boundsView.size.height = NHKBH+((NHKBH-35)/4+2);
        self.frame = boundsView;
        //适配界面
        if (self.superview.frame.size.height<NHKBH+((NHKBH-35)/4+2))
        {
            self.superview.frame=CGRectMake(self.superview.frame.origin.x, self.superview.frame.origin.y-((NHKBH-35)/4+2), self.superview.frame.size.width, NHKBH);
        }
        
    }else{
        charSets =  moreSymbols;
        rangs = @[@10,@19,@26];
        self.frame = CGRectMake(0, 0, NHSCREEN_WIDTH, NHKBH);
        self.superview.frame=CGRectMake(self.superview.frame.origin.x, self.superview.frame.origin.y+((NHKBH-35)/4+2), self.superview.frame.size.width, NHKBH);
    }
    NSLog(@"%@%@",self.superview,NSStringFromCGRect(self.frame));
    //第一排
    ...
    //第几行
    NSInteger nowNub=0;
    NSInteger length = [[rangs objectAtIndex:nowNub] integerValue];
    NSArray *chars = [charSets subarrayWithRange:NSMakeRange(loc, length)];
    //NSLog(@"第一排:%@",chars);
    NSInteger len = [chars count];
    CGFloat char_h_dis = 2;
    CGFloat char_v_dis = 5;
    ...
    UIFont *titleFont = [UIFont systemFontOfSize:NHKBFontSize];
    UIColor *titleColor = NHColor(71, 86, 115);
//    UIColor *bgColor = NHColor(249, 252, 253);
//    UIImage *bgImg = [UIImage pb_imageFromColor:bgColor];
    CGFloat cur_y = char_uper_dis;
    
    int n = 0;
    UIImage *charbgImg = [UIImage imageNamed:@"normal"];
    for (int i = 0 ; i < len; i ++) {
    ...
    if (!self.showSymbol) {
        ...
        loc = [[rangs objectAtIndex:nowNub-1] integerValue];
        length = [[rangs objectAtIndex:nowNub] integerValue];
...
//    CGFloat specialChar_width=(NHSCREEN_WIDTH-char_h_dis*4-shiftWidth*2-char_h_dis*(len-1))/len;
//    char_width = (NHSCREEN_WIDTH-char_h_dis*4-shiftWidth*2-char_h_dis*(len-1))/len;
    ...
    if (init) {
        ...
        [btn setBackgroundImage:[UIImage imageNamed:@"Caps-p"] forState:UIControlStateNormal];
        ...
        [btn setBackgroundImage:[UIImage imageNamed:@"Backspace"] forState:UIControlStateNormal];
        btn.contentMode=UIViewContentModeScaleAspectFill;
        btn.clipsToBounds=YES;
        [btn setBackgroundImage:[UIImage imageNamed:@"Backspace-p"] forState:UIControlStateHighlighted];
        [btn addTarget:self action:@selector(charDeleteAction:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:btn];
        self.deleteCharBtn=btn;
    }
    else
    {
        bounds = CGRectMake(char_h_dis*0.5, cur_y, shiftWidth, char_heigh);
        self.shiftBtn.frame = bounds;
        bounds = CGRectMake(NHSCREEN_WIDTH-char_h_dis*0.5-shiftWidth, cur_y, shiftWidth, char_heigh);
        self.deleteCharBtn.frame = bounds;
...
//        btn.backgroundColor=[UIColor redColor]
    ...
    if (init) {
        cur_y += char_heigh+char_v_dis;
        CGFloat numWidth = (NHSCREEN_WIDTH-2*char_h_dis-4.043*char_heigh)/2.0f;
        ...
        UIImage *charImg = [UIImage pb_imageFromColor:NHColor(196, 203, 216)];
        UIImage *roundcharImg = [charImg pb_drawRectWithRoundCorner:NHCHAR_CORNER toSize:CGSizeMake(numWidth, char_heigh)];
        [btn setTitle:@"符号" forState:UIControlStateNormal];
        [btn setBackgroundImage:roundcharImg forState:UIControlStateNormal];
        ...
        btn.titleLabel.textColor = [UIColor whiteColor];
        [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [btn setTitle:self.selectedBtnTitle forState:UIControlStateNormal];
        ...
        [self addSubview:btn];
        self.loginBtn=btn;
        bounds = CGRectMake(char_h_dis*1+numWidth, cur_y, 4.043*char_heigh, char_heigh);
        btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = bounds;
        btn.exclusiveTouch = true;
        btn.titleLabel.font = titleFont;
        ...
        btn.titleLabel.textColor = titleColor;
        [btn setTitleColor:titleColor forState:UIControlStateNormal];
        [btn setBackgroundImage:[UIImage imageNamed:@"Space"] forState:UIControlStateNormal];
        btn.contentMode=UIViewContentModeScaleAspectFill;
        btn.clipsToBounds=YES;
    ...
    else
    {
        ...
        CGFloat numWidth = (NHSCREEN_WIDTH-2*char_h_dis-4.043*char_heigh)/2.0f;
        bounds = CGRectMake(char_h_dis*0.5, cur_y, numWidth, char_heigh);
        self.charSymSwitch.frame = bounds;
        bounds = CGRectMake(NHSCREEN_WIDTH-char_h_dis*0.5-numWidth, cur_y, numWidth, char_heigh);
        self.loginBtn.frame = bounds;
        bounds = CGRectMake(char_h_dis*1+numWidth, cur_y, 4.043*char_heigh, char_heigh);
        self.safeBtn.frame = bounds;
...
- (void)shiftAction:(UIButton *)btn {
    if (self.showSymbol) {
        ...
        [self characterTouchAction:btn];
    }else{
        self.shiftEnable = !self.shiftEnable;
        NSArray *subChars = [self subviews];
        [btn setBackgroundImage:[UIImage imageNamed:self.shiftEnable?@"Caps":@"Caps-p"] forState:UIControlStateNormal];
...
- (void)updateShiftBtnTitleState {
    ...
    if (self.showSymbol) {
        title = @"+";
        [self.shiftBtn setBackgroundImage:[UIImage imageNamed:@"normal"] forState:UIControlStateNormal];
        
    }else{
        
        [self.shiftBtn setBackgroundImage:[UIImage imageNamed:self.shiftEnable?@"Caps":@"Caps-p"] forState:UIControlStateNormal];
...
//    if (CGRectContainsPoint(self.charSymSwitch.frame, touchPoint)) {
//        [self.charSymSwitch addPopup];
//    }
}
...
- (CGImageRef)createKeytopImageWithKind:(int)kind
{
    ...
    
    {
        ...
        83.0 / 255.0, 163.0 / 255.0, 237.0 / 255.0, 1.00,
        
        62.0 / 255.0, 136.0 / 255.0, 218.0 / 255.0, 1.00,
    ...
    CGGradientCreateWithColorComponents(rgb, colors, NULL, count);
...
- (void)addPopup {
    ...
    if ([self.chars isEqualToString:@"q"]||[self.chars isEqualToString:@"a"]||[self.chars isEqualToString:@"1"]) {
    ...
    else if ([self.chars isEqualToString:@"p"]||[self.chars isEqualToString:@"l"]||[self.chars isEqualToString:@"0"]) {
